...
**Game-master access goes through `scripts/lib/game-master.mjs`.** Five fetchers read the
...
cache and the shared parsers live in that one file. Do not re-declare a mirror URL in a fetcher,
and do not add a new species-fact upstream before checking whether the game master already
publishes it — [`docs/upstream-sources.md`](docs/upstream-sources.md) is the inventory, and it
records what nine months of a silently-stale upstream cost.
