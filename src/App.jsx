...
// Species refinement axes (regional forms / gender slots / un-searchable
// slots) and the shared badge row every chip surface renders. Re-exported
// below so the offline checks keep importing them from App.jsx.
import {
	AXIS_FORM,
	AXIS_GENDER,
	AXIS_SLOT,
...
// lineSlotCovered instead, so this returns false for them.
...
//
// `scopeOf` (dex → 'family' | 'upward' | 'exact', see upwardReach) narrows
// what each owned copy stands for. Explicitly scoped copies are literal: they
// fill exactly the dex they name, split rules or not (an 'exact' Waumpel DOES
// fill the Waumpel slot — the user said this copy is just a Waumpel), and
// they take no part in the line rules, so an 'exact' Bibor leaves Hornliu on
// the ask. Omitted, every copy is family-scoped and the answer is the one this
// function has always given.
//
// `ownedRoots` is the FAMILY-scoped have-list's line roots, hoisted out of
// the loop by callers that ask this for hundreds of candidates; it is derived
// on the spot when omitted. Exported for the offline checks in
// scripts/check-friend-collect.mjs.
export function lineSlotCovered(dex, ownedDex, ownedRoots = null, scopeOf = null) {
	let familyOwned = ownedDex;
	if (scopeOf) {
		familyOwned = new Set();
		for (const d of ownedDex) {
			const scope = scopeOf(d);
			if (scope === 'family') familyOwned.add(d);
			else if (explicitScopeReaches(d, scope, dex)) return true;
		}
	}
	const fam = SPLIT_FAMILY_BY_DEX.get(dex);
	if (fam) {
		const branch = fam.branches.find((b) => b.includes(dex));
		if (branch) return branch.some((d) => familyOwned.has(d));
		return fam.branches.every((b) => b.some((d) => familyOwned.has(d)));
	}
	if (familyOwned.has(dex)) return true;
	if (BABY_DEX.has(dex)) return false;
	const roots = ownedRoots || new Set([...familyOwned].map(lineRootDex));
...
// curated collect chips — the have-lists render as `+name`, so the lucky that
// fills a target's slot is worth naming on the chip. Rules live in
// lineSlotCovered above; this only picks the member out, and scans the
// have-list to do it, so it is the display path only — the packs ask the
// boolean. Exported for the offline checks.
export function lineSlotOwner(dex, ownedDex, scopeOf = null) {
	if (!lineSlotCovered(dex, ownedDex, null, scopeOf)) return null;
	...
	if (scopeOf) {
		...
		for (const d of ownedDex) {
			const scope = scopeOf(d);
			if (scope === 'family') familyOwned.add(d);
			else if (explicitScopeReaches(d, scope, dex)) return d;
		}
	}
	const fam = SPLIT_FAMILY_BY_DEX.get(dex);
	if (fam) {
		const pool = fam.branches.find((b) => b.includes(dex)) || fam.branches.flat();
		return pool.find((d) => familyOwned.has(d)) ?? null;
	}
...
export const DEFAULT_CONFIG = {
	...
	//   friendCollectDropSlots: { species: [slotKey, …] } — the un-searchable
	//     slots (INVISIBLE_FORM_SLOTS: Burmy cloaks, Sesokitz seasons, …) the
	//     friend should NOT hunt. The have-list mirror of this is
	//     hundoSlots/luckySlots, and like those it can never become a guard —
	//     PoGo search cannot name these forms at all. Its whole effect is on
	//     COVERAGE: the target only counts as done once every slot still
	//     asked for is ticked on the matching have-list. Only species in the
	//     INVISIBLE_FORM_SLOTS catalog.
...
export function mergeImportedConfig(raw, notices = []) {
	...
	// Un-searchable slot annotation values: dedupe against the species' own slot
	// list. Declared here (rather than beside hundoSlots below) because the
	// friend-collect drop map validates against it too.
	...
	merged.friendCollectDropSlots = canonMapKeys(merged.friendCollectDropSlots, (key, v) => {
		if (!friendCollectSet.has(key)) return undefined;
		const keep = validSlotKeys(key, v);
		if (!keep) return undefined;
		// Dropping every slot would ask for nothing at all — the picker blocks
		// it, so a full drop here can only be import junk.
		return keep.length < (invisibleSlotsFor(key)?.slots.length || 0) ? keep : undefined;
	});
...
export function buildFilters(
	...
	// dex → line scope for one have-list (cfg.luckyScope / cfg.hundoScope, keyed
	// by the have-list entry). Two entries resolving to one dex — a locale
	// duplicate — keep the wider scope, so a stray narrow twin cannot silently
	// re-open a line the other entry closed.
	const SCOPE_WIDTH = { exact: 0, upward: 1, family: 2 };
	const ownedScopeByDex = (names, scopeMap) => {
		const out = new Map();
		for (const n of names || []) {
			const dex = resolveSpeciesInfo(n)?.dex;
			if (!dex) continue;
			const scope = haveScopeOf(scopeMap, canonKey(n));
	...
	//
	// Un-searchable slots (Burmy cloaks, Sesokitz seasons, …) join on the same
	// terms, and close the last gap between the have-list chips and these ones:
	// the have-lists could already say "my lucky Burmy is the plant cloak", but
	// the curated wishlist then counted the whole species as done and stopped
	// asking friends for the other three. Now a target whose slots are only
	// partly owned stays on the ask — which is exactly what the automatic lucky
	// wishlist has always done (see invisibleSlotsIncomplete, where withholding
	...
	// Both have-lists are walked once here, not once per candidate — the packs
	// ask this question for hundreds of species on every rebuild. `roots` are
	// the FAMILY-scoped copies' line roots only, which is what lineSlotCovered
	// expects when a scope lookup rides along.
	const ownedLine = (names, slotAnn, scopeMap) => {
		const dexSet = ownedDexSet(names);
		const scopeOf = scopeLookup(ownedScopeByDex(names, scopeMap));
		return {
			dexSet,
			scopeOf,
			roots: new Set([...dexSet].filter((d) => scopeOf(d) === 'family').map(lineRootDex)),
			slotGaps: incompleteSlotsByRoot(slotAnn),
		};
	};
	const friendCollectLuckyLine = ownedLine(luckies, cfg.luckySlots, cfg.luckyScope);
	const friendCollectHundoLine = ownedLine(hundos, cfg.hundoSlots, cfg.hundoScope);
	const friendCollectLineOwned = (dex, { dexSet, scopeOf, roots, slotGaps }) => {
		if (!dex) return false;
		...
		if (SPLIT_FAMILY_BY_DEX.has(dex)) return lineSlotCovered(dex, dexSet, roots, scopeOf);
		if (dexSet.has(dex)) return true;
		if (slotGaps.has(lineRootDex(dex))) return false;
		return lineSlotCovered(dex, dexSet, roots, scopeOf);
	};
	// Display-only mirror for the curated chips: the have-list member that fills
	// a target's line, when it isn't the target species itself. Same gates as
	// the boolean (babies, coin-flip branches, un-searchable slots, scopes), so
	// one rule feeds both — this one just names the mon instead of pruning.
	const friendCollectLineOwner = (dex, { dexSet, scopeOf, slotGaps }) => {
		if (!dex || dexSet.has(dex)) return null;
		if (slotGaps.has(lineRootDex(dex))) return null;
		const owner = lineSlotOwner(dex, dexSet, scopeOf);
	...
	if (friendCollectActive.length > 0) {
		...
		//
		// Dropped SLOTS emit nothing, and can't: PoGo search has no keyword for
		// a Burmy cloak or a Sesokitz season. They have already done their work
		// upstream, in friendCollectGoalOwned — narrowing what must be owned
		// before the target leaves the string at all.
...
export default function App() {
	...
	function detectFriendCollectCovered(added, goal) {
		...
		// Line-level, like buildFilters' friendCollectCovered: a freshly added
		// lucky Bibor retires a curated Hornliu. The new entry has no scope yet
		// (badges are click-only), so it counts for its whole line; the OTHER
		// have-list is read with its scopes, as the filter reads it.
		const otherScope = goal === 'hundo' ? config.luckyScope : config.hundoScope;
		const otherDex = new Set([...otherSet].map((n) => resolveSpeciesInfo(n)?.dex).filter(Boolean));
		const otherScopeOf = (d) => haveScopeOf(otherScope, pokemonNameFor(String(d)));
		const notices = [];
		const seen = new Set();
		for (const sp of added) {
			...
			for (const target of curated) {
				...
				if (!tdex || seen.has(target)) continue;
				if (!lineSlotCovered(tdex, new Set([dex]))) continue;
				if (!genderSatisfied(thisAnn, target)) continue;
				seen.add(target);
				const nowFullyCovered =
					mode === 'both'
						? lineSlotCovered(tdex, otherDex, null, otherScopeOf) && genderSatisfied(otherAnn, target)
						: true;
				notices.push({ species: target, via: target === sp ? null : sp, goal, nowFullyCovered });
			}
		}
		return notices;
	}
	...
	function removeHundo(h) {
		...
		setConfig((prev) => {
			...
			if (prev.hundoScope?.[h]) next.hundoScope = omitKey(prev.hundoScope, h);
			return next;
		});
	}
	function addLucky() {
	...
	function removeLucky(s) {
		...
		setConfig((prev) => {
			...
			if (prev.luckyScope?.[s]) next.luckyScope = omitKey(prev.luckyScope, s);
			return next;
		});
	}
...
// The have-list chip add-on: one refinement row per chip, whatever axis the
// species happens to carry. Click-only — typed input never sets an annotation,
// and no badge active = unknown = species-level behaviour everywhere (exactly
// the pre-annotation semantics).
//
// All three axes record what you OWN, additively, into their own config map:
//   form   → hundoForms   / luckyForms   — which regional form(s) this copy is.
//            Feeds the friend-collect coverage predicate and the form-scoped
//            `!+family` exclusions in the fallback wishlists.
//   gender → hundoGenders / luckyGenders — for GENDER_SLOT_DEX species, where
//            the wrong gender is a dead end. Which genders CLOSE the slot is
//            the catalog's call, so a recorded-but-not-closing pick stays blue
//            and only a complete set turns amber.
...
function HaveScopeBadge({ species, scopeAnn, onScopeAnnChange, accent = '#5EAFC5', t }) {
	...
	const scope = haveScopeOf(scopeAnn, species);
	const next = HAVE_SCOPES[(HAVE_SCOPES.indexOf(scope) + 1) % HAVE_SCOPES.length];
	const cycle = () => {
		if (next === 'family') onScopeAnnChange(omitKey(scopeAnn, species));
		else onScopeAnnChange({ ...(scopeAnn || {}), [species]: next });
	};
...
function HaveAxisBadges({
	...
	const entry = refinementAxisFor(species);
	if (!entry) return null;
	const [ann, onChange, helpKey] = {
		[AXIS_FORM]: [formsAnn, onFormsAnnChange, 'app.have_forms.help'],
		[AXIS_GENDER]: [gendersAnn, onGendersAnnChange, 'app.have_genders.help'],
		[AXIS_SLOT]: [slotsAnn, onSlotsAnnChange, 'app.have_slots.help'],
	}[entry.axis];
	if (!onChange) return null;
	const owned = new Set(ann?.[species] || []);
	// Amber = this axis is finished, so "done" reads differently from "recorded
	// but still hunting". Forms stay blue: a species-level lucky is never
	// "complete" just because every catalog form is ticked — the wishlists
	// scope by form rather than closing a slot.
	const complete = entry.axis !== AXIS_FORM && refinementComplete(entry, owned);
	const toggle = (key) => {
		const next = new Set(owned);
		if (next.has(key)) next.delete(key);
		else next.add(key);
		if (next.size > 0) onChange({ ...(ann || {}), [species]: [...next] });
		else onChange(omitKey(ann, species));
	};
	return (
		<RefinementBadges
			entry={entry}
			t={t}
			title={t(helpKey)}
			tint={complete ? '#E2B93B' : '#5EAFC5'}
			stateFor={(key) => (owned.has(key) ? 'on' : 'off')}
			onToggle={toggle}
			ringFor={entry.axis === AXIS_SLOT ? (key) => key === activeSlot : undefined}
			titleFor={
				entry.axis === AXIS_SLOT
					? (key) => (key === activeSlot ? t('app.have_slots.spawning_now') : undefined)
					: undefined
			}
		/>
	);
...
function FriendCollectEditor({
	...
	function remove(name) {
		...
		if (onDropSlotsChange && name in dropSlots) onDropSlotsChange(omitKey(dropSlots, name));
	}
	function clearAll() {
		onChange([]);
		...
		if (onDropSlotsChange && Object.keys(dropSlots).length > 0) onDropSlotsChange({});
	}
	...
	// string, while a dropped slot can never be expressed in PoGo search at
	// all and instead narrows what has to be owned before the target counts
	// as done. Both are stored the same way, so the toggle doesn't care.
	const dropStateFor = (axis) =>
		axis.axis === AXIS_SLOT ? [dropSlots, onDropSlotsChange] : [dropForms, onDropFormsChange];
	function dropListFor(name, axis) {
	...
	return (
		<div className='space-y-3'>
			...
			{/* Only worth saying once a curated target actually carries un-searchable
			    slots — for everyone else it is a paragraph about nothing. */}
			{targets.some((tg) => tg.slotsOwned) && (
				<p className='mono text-[10.5px] text-[#8090A0] leading-relaxed'>
					{t('app.filter.friend_collect_slots_note')}
				</p>
			)}
			...
			{targets.length > 0 ? (
				...
				{targets.map((tg) => {
					...
					// The species' own refinement axis, if it has one. Gender-axis
					// species need no second row — the universal ♀/♂ lock above IS
					// their axis — so only forms and slots render here.
					const axis = refinementAxisFor(tg.species);
					const dropAxis = axis && axis.axis !== AXIS_GENDER ? axis : null;
					return (
						<span
							key={tg.species}
							title={
						...
						>
							...
							{onGendersChange && (
								<RefinementBadges
									...
									t={t}
									title={t('app.filter.friend_collect_gender_help')}
									stateFor={(g) => (genders[tg.species] === g ? 'on' : 'off')}
									onToggle={(g) => toggleGender(tg.species, g)}
								/>
							)}
							{dropAxis && (
								<RefinementBadges
									...
									titleFor={
										dropAxis.axis === AXIS_SLOT
											? (key) =>
													tg.slotsOwned?.includes(key)
														? t('app.filter.friend_collect_slot_owned')
														: undefined
											: undefined
									}
								/>
			...
			>
				<div className='space-y-4'>
					...
					<FriendCollectEditor
						...
						dropSlots={config.friendCollectDropSlots || {}}
						onDropSlotsChange={(next) => set('friendCollectDropSlots', next)}
					/>
				</div>
			</Collapsible>
				...
				<div className='flex flex-col gap-1.5'>
                    ...
                    `!<species>,<gender>` guard. */}
								<RefinementBadges
									entry={GENDER_LOCK_AXIS}
									t={t}
									title={t('app.buddy_targets.gender_help')}
									size='sm'
									tint='#E67E22'
									stateFor={(g) => (tg.gender === g ? 'on' : 'off')}
									onToggle={(g) => updateAt(i, { gender: tg.gender === g ? 'any' : g })}
								/>
