...
export function buildFilters(
	...
	const pushOwnedExclusions = (clauses, names, scopedMap, genderAnn, slotAnn, whyKeys) => {
		...
		// Evolution-line ROOTS with an un-searchable slot still unfilled, mapped to
		// the unfinished species themselves. Keyed by root rather than by species
		// name because `!+X` expands to the whole candy family: owning a
		// Kronjuwild would otherwise emit `!+kronjuwild` and hide the Sesokitz
		// whose seasons are still incomplete, defeating the withholding entirely.
		// One unfilled slot suppresses the plain exclusion for every member of
		// that family — but only the plain one; see the form branch below.
		const slotIncompleteByRoot = new Map();
		for (const key of Object.keys(slotAnn || {})) {
			...
			if (!slotIncompleteByRoot.has(root)) slotIncompleteByRoot.set(root, []);
			slotIncompleteByRoot.get(root).push(invisibleSlotsFor(key));
		}
