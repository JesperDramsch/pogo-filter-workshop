...
// Type 3 paired (Zangoose/Seviper, Lunatone/Solrock) and Type 4 hemispheric
// (Sawk/Throh, Heatmor/Durant) — KMZ ships only the dividing LineString
// (the "Type 3 [Paired Regional Line]" L-shape through the Atlantic and
// Mediterranean down through Iran/Indian Ocean), which pointInRegionGeom
// can't hit-test. We synthesize East/West polygons from that line so that
// Africa, the Americas, and Greenland share one side and Europe / Asia /
// Oceania share the other — matching Niantic's actual split. Species
// assignments are keyed to the current rotation; update when Niantic
// announces a swap (Leek Duck / GO Hub post the day-of).
// Last verified May 2026.
// Polygons trace the KMZ paired-regional L-line. Intermediate vertices along
// the top/bottom edges keep every consecutive lon step under 180°, so the
// unwrapRing/shiftPointToRing logic doesn't flip the polygon inside-out
...
const POGO_REGIONS_ROTATING = [
  {
    folder: "Type 3/4 [Rotating Pairs] (current rotation, manually maintained)",
    name: "Eastern paired/hemispheric (Europe + Asia + Oceania)",
    english: ["Zangoose", "Sawk",      "Lunatone",  "Heatmor"],
    german:  ["Sengo",    "Karadonis", "Lunastein", "Furnifraß"],
    geometry: { type: "Polygon", coordinates: [PAIRED_LINE.east] },
  },
  {
    folder: "Type 3/4 [Rotating Pairs] (current rotation, manually maintained)",
    name: "Western paired/hemispheric (Americas + Africa)",
    english: ["Seviper", "Throh",    "Solrock", "Durant"],
    german:  ["Vipitis", "Jiutesto", "Sonnfel", "Fermicula"],
