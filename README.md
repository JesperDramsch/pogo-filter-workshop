...
- **Event spawns, egg pool & field-research rewards** from [bigfoott/ScrapedDuck](https://github.com/bigfoott/ScrapedDuck) (events/eggs/research feeds). Powers the "Events" shiny-grind filters — featured wild spawns (Community Day / Spotlight Hour), the current egg pool, and research rewards, each as a paste-able family search of shiny-eligible species. Snapshot at `src/data/event-spawns.json`, refreshed by `npm run fetch-event-spawns` and the daily `.github/workflows/sync-event-spawns.yml`. Note: generic "event"-type weeks only flag `hasSpawns` upstream without listing the species, so their wild spawns can't be surfaced — eggs and research (which reflect the active event's pools) still are.
