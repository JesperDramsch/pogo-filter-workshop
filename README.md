...
- **Species pools for the friend-collect packs** (special-trade classes, starters, power lines, evolution parents) from the [PokeMiners game master](https://github.com/PokeMiners/game_masters) unioned with [PoGoAPI.net](https://pogoapi.net/). The **Mega-capable roster** comes from the game master alone — it's the only source that separates a Mega from a Primal Reversion. Snapshot at `src/data/species-meta.json`, refreshed by `npm run fetch-species-meta` and the daily `sync-species-meta` workflow — so a newly released Mega Evolution joins the "Mega evolutions" pack without a code change.
